...
async function registerWonderCraftServiceWorker_(){
  ...
  try{
    const reg = await navigator.serviceWorker.register(
      "./service-worker.js?v=7.52.2-commute-result-sync",
      { updateViaCache:"none" }
    );
...
async function apiPost(action,payload){
  const run=async()=>{
    ...
    try{
      ...
      const error=new Error(
        looksHtml
          ?"サーバー処理が途中で終了しました。自動でもう一度試しましたが完了しませんでした。"
  ...
  try{
    ...
    if(error?.code!=="API_INVALID_RESPONSE")throw error;
...
function switchView(view){
  ...
  // WC-7.51.6: 案件検索を開いた時は常勤案件を基本モードにする。
...
async function runStationAwareJobSearch_(options={}){
  ...
  (jobSearchItems||[]).forEach((job,index)=>{
    if(!job.__wcCommuteId) job.__wcCommuteId=`WC7522-${Number(job.rowNumber||0)}-${String(job.shopName||"").normalize("NFKC").replace(/\s+/g,"").slice(0,24)}`;
  });
    ...
    while(true){
      ...
      try{
        ...
        rows.forEach(row=>{
          ...
          if(normalizedRow.minutes!==null && normalizedRow.minutes!==undefined && normalizedRow.minutes!==""){
            const n=Number(normalizedRow.minutes);
            normalizedRow.minutes=Number.isFinite(n)&&n>0?n:null;
          }else{
            normalizedRow.minutes=null;
          }
          jobStationCommuteMap[String(normalizedRow.rowNumber)]=normalizedRow;
          if(normalizedRow.clientCommuteId){
            jobStationCommuteMap[String(normalizedRow.clientCommuteId)]=normalizedRow;
          }
          const target=(jobSearchItems||[]).find(job=>
            (normalizedRow.clientCommuteId && normalizedRow.clientCommuteId===job.__wcCommuteId) ||
            Number(job.rowNumber)===Number(normalizedRow.rowNumber)
          );
          if(target){
            target.__wcCommute=normalizedRow;
            if(normalizedRow.ok===true && normalizedRow.minutes!==null){
              target.commuteMinutes=normalizedRow.minutes;
            }
          }
          row=normalizedRow;
...
function wcRankingBreakdownHtml_(job, safeRanking){
  ...
  const commuteRaw=ranking.commuteMinutes ?? job?.__wcCommute?.minutes ?? job.commuteMinutes;
  const commuteMinutes=
    commuteRaw!==null && commuteRaw!==undefined && commuteRaw!=="" &&
    Number.isFinite(Number(commuteRaw)) && Number(commuteRaw)>0
      ? Number(commuteRaw)
      : null;
  const commuteDetail=commuteMinutes!==null
...
function wcSpotCardHtmlV7510_(x,originStation,maxMinutes){
  const routeInfo=x.__wcCommute||jobStationCommuteMap[String(x.__wcCommuteId||"")]||jobStationCommuteMap[String(x.rowNumber)]||null;
...
function renderJobSearchResults(){
  ...
  let items=jobSearchItems.filter(x=>{
    ...
    // WC-7.51.6: 未実測案件はここで落とさず、通勤要確認として残す。
  ...
  items=items.map(x=>{
    const prepared=wcPrepareJobSearchItem_(x);
    const routeInfo=prepared.__wcCommute||jobStationCommuteMap[String(prepared.__wcCommuteId||"")]||jobStationCommuteMap[String(prepared.rowNumber)]||null;
    const commute=originStation
      ? getValidJobCommuteMinutes_(routeInfo)
      : (()=>{const m=Number(getJobCommuteMinutes_(prepared));return Number.isFinite(m)&&m>0?m:null;})();

    prepared.__wcSafeRanking=
      wcSafeJobRankingV7437_(
        prepared,
        commute,
        maxMinutes,
        min,
    ...
    return prepared;
  }).filter(x=>!x.__wcSafeRanking?.excluded);
  ...
  if(qTokens.length){
    items.sort(
      (a,b)=>
        wcJobKeywordScore_(b,qTokens)-wcJobKeywordScore_(a,qTokens)||
    ...
    items.sort(
      (a,b)=>
        wcPrepareJobSearchItem_(b).__wcPay-
        wcPrepareJobSearchItem_(a).__wcPay
    );
  }else if(sort==="name"){
    items.sort(
      (a,b)=>
    ...
    /*
     * 「更新日が新しい順」を選んでいても、
     * WC-7.43.7ではおすすめ点を最優先。
     * 同点時だけ新しい案件を上へ。
     */
    items.sort(
      (a,b)=>
        Number(b.__wcSafeRanking?.score||0)-
        Number(a.__wcSafeRanking?.score||0)||
  ...
  box.innerHTML=items.map(x=>{
    ...
    // WC-7.51.6: スポットは長期案件用の採点UIを使わない。
...
function wcSpotScheduleDisplayV7512_(job){
  // 明示的な稼働日フィールドを最優先。更新日はここでは使わない。
  for(const value of [
    job?.workDate,job?.eventDate,job?.shiftDate,job?.scheduleDate,
    job?.workDates,job?.eventDates
  ]){
    const s=wcSpotScheduleTextV7512_(value);
    if(s)return s;
  }

  const text=[job?.originalText,job?.remarks,job?.description,job?.detail,job?.shopName]
    .filter(Boolean).join("\n").normalize("NFKC");
  if(!text)return "";

  // ラベル付き日程を最優先。
  const labeled=text.match(/(?:稼働日程|稼働日|日程|実施日|勤務日|開催日|勤務期間|期間)\s*[：:]\s*([^\n\r]{1,120})/i);
  if(labeled){
    const s=wcSpotScheduleTextV7512_(labeled[1]);
    if(s)return s;
  }

  // 原文中の日付は、勤務・催事等の語が近くにある場合だけ採用。
  const dateRe=/(?:20\d{2}[\/.\-年]\d{1,2}[\/.\-月]\d{1,2}日?|\d{1,2}月\d{1,2}日|\d{1,2}[\/.\-]\d{1,2})(?:\s*[\(（][月火水木金土日祝][\)）])?/g;
  const matches=[...text.matchAll(dateRe)];
  for(const m of matches){
    const pos=m.index||0;
    const around=text.slice(Math.max(0,pos-45),Math.min(text.length,pos+m[0].length+45));
    if(/稼働|勤務|日程|実施|開催|催事|イベント|入店|シフト|期間/i.test(around)){
      const s=wcSpotScheduleTextV7512_(around);
      if(s)return s;
    }
  }
  return "";
...
function wcSpotScheduleTextV7512_(value){
  ...
  if(dates.length){
    const normalized=dates.map(v=>v
      .replace(/20\d{2}[\/.\-年](\d{1,2})[\/.\-月](\d{1,2})日?/,"$1/$2")
      .replace(/(\d{1,2})月(\d{1,2})日/,"$1/$2")
      .replace(/(\d{1,2})[.\-](\d{1,2})/,"$1/$2"));
    const unique=[...new Set(normalized)];
    return unique.slice(0,5).join("・")+(unique.length>5?" ほか":"");
  }

  if(/応相談|要相談|随時|毎週|平日|土日|週末|シフト/i.test(s)){
...
function getJobDate_(job){
   ...
   */
  return wcSpotScheduleDisplayV7512_(job);
}
...
function testSafeJobRankingV7437_(){
  const tests=[
    {minutes:60,expectedExcluded:false},
    {minutes:75,expectedExcluded:false},
    {minutes:105,expectedExcluded:false},
    {minutes:120,expectedExcluded:false},
    {minutes:121,expectedExcluded:true}
  ].map(item=>{
    const result=wcSafeJobRankingV7437_(
      {
        __wcPay:15000,
        experienceClass:"未経験可"
      },
      item.minutes,
      60,
      18000,
    ...
    return {
      ...
      expectedExcluded:item.expectedExcluded,
      score:result.score,
      passed:result.excluded===item.expectedExcluded
    };
  });

  const output={
    passed:tests.every(x=>x.passed),
    tests
  };
